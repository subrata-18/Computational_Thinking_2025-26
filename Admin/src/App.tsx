      ...
      <main className="admin-main">
        ...
        {view === "detail" && detail && (
          <section className="workspace">
            ...
            <section className="detail-section"><div className="section-heading"><h2>Question review</h2><span>{parseStoredList(detail.ai_questions).length} questions</span></div><div className="review-list">{parseStoredList(detail.ai_questions).map((question, index) => {
              const isWrong = parseStoredList(detail.wrong_answered_question ?? "").some((wrongQuestion) => question.includes(wrongQuestion) || wrongQuestion.includes(question.substring(0, 30)));
              return <article className="review-item" key={`${index}-${question}`}><div className={`review-status ${isWrong ? "incorrect" : "correct"}`} aria-label={isWrong ? "Incorrect" : "Correct"}>{isWrong ? "\u00d7" : "\u2713"}</div><div className="review-content"><div className="review-number">Question {index + 1}</div><div className="review-question">{question}</div><div className="review-answer">Correct answer: <strong>{parseStoredList(detail.ai_answers)[index] || "Not recorded"}</strong></div></div></article>;
            })}</div></section>
